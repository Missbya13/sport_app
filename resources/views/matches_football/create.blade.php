...
@section('content')
<div class="container">
    <h1>Créer un match de football</h1>

    <form action="{{ route('matches_football.store') }}" method="post">
        @csrf
        <div class="mb-3">
            <label for="teamA_id" class="form-label">Équipe A :</label>
            <select name="teamA_id" id="teamA_id" class="form-control" required>
        ...
        <div class="mb-3">
            ...
            <select name="teamB_id" id="teamB_id" class="form-control" required>
                @foreach($teams as $team)
                    <option value="{{ $team->id }}">{{ $team->name }}</option>
                @endforeach
            </select>
